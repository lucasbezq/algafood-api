...
import com.algaworks.algafood.api.v1.converter.FormaPagamentoDTOConverter;
import com.algaworks.algafood.api.v1.dto.FormaPagamentoDTO;
import com.algaworks.algafood.api.v1.dto.request.FormaPagamentoRequest;
...
public class FormaPagamentoController implements FormaPagamentoOpenApi {
    ...
    public ResponseEntity<FormaPagamentoDTO> buscar(@PathVariable Long formaPagamentoId, ServletWebRequest request) {
        ...
        String eTag = "0";
        var dataUltimaAtualizacao = formaPagamentoRepository.getDataAtualizacaoById(formaPagamentoId);
        if (dataUltimaAtualizacao != null) eTag = String.valueOf(dataUltimaAtualizacao.toEpochSecond());
        if (request.checkNotModified(eTag)) return null;
